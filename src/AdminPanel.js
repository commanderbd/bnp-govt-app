...
export default function AdminPanel({ onLogout, isDark, T }) {
  ...
  async function fetchAll() {
    setLoading(true);
    const [m, n, mp, p, dec, doc, fb] = await Promise.all([
  ...
  return (
    <div style={{ fontFamily: "sans-serif", background: T.bg, minHeight: "100vh", color: T.text }}>
      ...
      <div style={{ display: "flex", background: T.navBg, borderBottom: `2px solid ${T.navBorder}`, overflowX: "auto" }}>
        {sections.map(s => (
          <button key={s.id} onClick={() => setActiveSection(s.id)} style={{ background: activeSection === s.id ? "rgba(201,168,76,0.15)" : "transparent", border: "none", borderBottom: activeSection === s.id ? "3px solid #C9A84C" : "3px solid transparent", color: activeSection === s.id ? "#C9A84C" : T.textMuted, padding: "12px 16px", cursor: "pointer", fontSize: 12, whiteSpace: "nowrap", fontFamily: "sans-serif" }}>
            {s.label}
          </button>
        ))}
      </div>
              ...
              {ministers.map((m, i) => (
                <div key={i} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 8, padding: 12, marginBottom: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                    ...
                    <button onClick={async () => { if (!window.confirm("মুছবেন?")) return; await supabase.from("ministers").delete().eq("id", m.id); showMessage("মুছে ফেলা হয়েছে"); fetchAll(); }} style={deleteBtnStyle}>🗑️</button>
                  </div>
                  <textarea placeholder="সংক্ষিপ্ত পরিচিতি (bio)" defaultValue={m.bio || ""}
                    onBlur={async e => { const bio = e.target.value.trim(); if (bio !== (m.bio || "")) { await supabase.from("ministers").update({ bio }).eq("id", m.id); showMessage(`${m.name}-এর bio আপডেট হয়েছে`); } }}
                  ...
                  <div style={{ display: "flex", gap: 6, marginTop: 6 }}>
                    <input placeholder="ফোন" defaultValue={m.phone || ""} onBlur={async e => { const phone = e.target.value.trim(); if (phone !== (m.phone || "")) await supabase.from("ministers").update({ phone }).eq("id", m.id); }} style={{ flex: 1, background: T.bg, border: `1px solid ${T.border}`, borderRadius: 6, padding: "5px 8px", color: T.text, fontSize: 11, fontFamily: "sans-serif" }} />
                    <input placeholder="ইমেইল" defaultValue={m.email || ""} onBlur={async e => { const email = e.target.value.trim(); if (email !== (m.email || "")) await supabase.from("ministers").update({ email }).eq("id", m.id); }} style={{ flex: 1, background: T.bg, border: `1px solid ${T.border}`, borderRadius: 6, padding: "5px 8px", color: T.text, fontSize: 11, fontFamily: "sans-serif" }} />
                  </div>
                </div>
              ))}
            </div>
          ...
          {activeSection === "news" && (
            <div>
              ...
              <div style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 10, padding: 16, marginBottom: 20 }}>
                ...
                <input placeholder="মূল সংবাদের লিংক (URL)" value={newNews.link} onChange={e => setNewNews({ ...newNews, link: e.target.value })} style={{ ...inputStyle, marginBottom: 14 }} />
                <button onClick={async () => {
              ...
              {news.map((n, i) => (
                <div key={i} style={{ background: T.card, border: `1px solid ${T.border}`, borderLeft: "4px solid #006A4E", borderRadius: 8, padding: 12, marginBottom: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8, marginBottom: 8 }}>
                    ...
                    <button onClick={async () => { if (!window.confirm("মুছবেন?")) return; await supabase.from("news").delete().eq("id", n.id); showMessage("মুছে ফেলা হয়েছে"); fetchAll(); }} style={deleteBtnStyle}>🗑️</button>
                  </div>
                  <textarea placeholder="সম্পূর্ণ সংবাদ লিখুন..." defaultValue={n.content || ""}
                    onBlur={async e => { const content = e.target.value.trim(); if (content !== (n.content || "")) { await supabase.from("news").update({ content }).eq("id", n.id); showMessage("সংবাদ আপডেট হয়েছে"); } }}
                  ...
                  <input placeholder="মূল সংবাদের লিংক (URL)" defaultValue={n.link || ""}
                    onBlur={async e => { const link = e.target.value.trim(); if (link !== (n.link || "")) { await supabase.from("news").update({ link }).eq("id", n.id); showMessage("লিংক আপডেট হয়েছে"); } }}
              ...
              {projects.map((p, i) => (
                <div key={i} style={{ background: T.card, border: `1px solid ${T.border}`, borderRadius: 8, padding: 14, marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                    ...
                    <button onClick={async () => { if (!window.confirm("মুছবেন?")) return; await supabase.from("projects").delete().eq("id", p.id); showMessage("মুছে ফেলা হয়েছে"); fetchAll(); }} style={deleteBtnStyle}>🗑️</button>
                  </div>
                  ...
                  <input type="range" min="0" max="100" value={p.progress}
                    onChange={async e => { await supabase.from("projects").update({ progress: Number(e.target.value) }).eq("id", p.id); fetchAll(); }}
              ...
              {decisions.map((d, i) => (
                <div key={i} style={{ background: T.card, border: `1px solid ${T.border}`, borderLeft: "4px solid #C9A84C", borderRadius: 8, padding: 12, marginBottom: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    ...
                    <button onClick={async () => { if (!window.confirm("মুছবেন?")) return; await supabase.from("decisions").delete().eq("id", d.id); showMessage("মুছে ফেলা হয়েছে"); fetchAll(); }} style={deleteBtnStyle}>🗑️</button>
                  </div>
                </div>
              ))}
            </div>
              ...
              {documents.map((d, i) => (
                <div key={i} style={{ background: T.card, border: `1px solid ${T.border}`, borderLeft: "4px solid #3B8BD4", borderRadius: 8, padding: 12, marginBottom: 8 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
                    ...
                    <button onClick={async () => { if (!window.confirm("মুছবেন?")) return; await supabase.from("documents").delete().eq("id", d.id); showMessage("মুছে ফেলা হয়েছে"); fetchAll(); }} style={deleteBtnStyle}>🗑️</button>
                  </div>
                </div>
              ))}
            </div>
                ...
                <div key={i} style={{ background: T.card, border: `1px solid ${f.status === "pending" ? "#E8593C" : T.border}`, borderLeft: `4px solid ${f.status === "approved" ? "#4ecba0" : "#E8593C"}`, borderRadius: 8, padding: 14, marginBottom: 10 }}>
                  <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 8 }}>
                    ...
                    <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
                      {f.status === "pending" && (
                        <button onClick={async () => { await supabase.from("feedback").update({ status: "approved" }).eq("id", f.id); showMessage("অনুমোদন করা হয়েছে!"); fetchAll(); }} style={{ background: "#006A4E", color: "#fff", border: "none", borderRadius: 6, padding: "4px 10px", cursor: "pointer", fontSize: 12 }}>✅ অনুমোদন</button>
                      )}
                      <button onClick={async () => { if (!window.confirm("মুছবেন?")) return; await supabase.from("feedback").delete().eq("id", f.id); showMessage("মুছে ফেলা হয়েছে"); fetchAll(); }} style={deleteBtnStyle}>🗑️</button>
                    </div>
                  </div>
