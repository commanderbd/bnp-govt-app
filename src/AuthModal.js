...
export default function AuthModal({ onClose, onSuccess, T, isDark }) {
  ...
  const inputStyle = { width: "100%", background: T.bg, border: `1px solid ${T.border}`, borderRadius: 8, padding: "12px 14px", color: T.text, fontSize: 14, boxSizing: "border-box", outline: "none", fontFamily: "sans-serif", marginBottom: 10 };
  ...
  async function verifyOTP() {
    ...
    try {
      ...
      if (error) throw error;
      if (data.user) { await supabase.auth.updateUser({ data: { full_name: name } }); onSuccess(data.user); onClose(); }
    } catch { setError("OTP ভুল হয়েছে"); }
    setLoading(false);
  }
    ...
    <div onClick={onClose} style={{ position: "fixed", top: 0, left: 0, right: 0, bottom: 0, background: "rgba(0,0,0,0.75)", zIndex: 500, display: "flex", alignItems: "center", justifyContent: "center", padding: 16 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: T.card, border: "2px solid #006A4E", borderRadius: 16, width: "100%", maxWidth: 380 }}>
        <div style={{ background: "linear-gradient(135deg, #006A4E, #004d38)", padding: "16px 20px", borderRadius: "14px 14px 0 0", display: "flex", justifyContent: "space-between", alignItems: "center" }}>
          <div>
            <div style={{ fontSize: 16, fontWeight: "bold", color: "#fff" }}>🇧🇩 লগইন / নিবন্ধন</div>
        ...
        <div style={{ padding: 20 }}>
          {step === "choose" && (
            <div>
              <div style={{ fontSize: 13, color: T.textMuted, marginBottom: 16, textAlign: "center" }}>নাম ও ইমেইল দিয়ে সহজেই অ্যাকাউন্ট তৈরি করুন</div>
              ...
              <div style={{ marginBottom: 14 }}>
                ...
                <input type="email" placeholder="example@email.com" value={email} onChange={e => setEmail(e.target.value)} onKeyDown={e => e.key === "Enter" && sendOTP()} style={inputStyle} />
              </div>
              ...
              <div style={{ fontSize: 11, color: T.textMuted, textAlign: "center", marginTop: 12, lineHeight: 1.6 }}>আপনার ইমেইলে একটি ৬ সংখ্যার কোড পাঠানো হবে</div>
            </div>
          )}
          {step === "otp" && (
            <div>
              <div style={{ textAlign: "center", marginBottom: 20 }}>
                ...
                <div style={{ fontSize: 12, color: T.textMuted }}><span style={{ color: "#C9A84C" }}>{email}</span> এ কোড পাঠানো হয়েছে</div>
              </div>
              <input placeholder="৬ সংখ্যার কোড" value={otp} onChange={e => setOtp(e.target.value.replace(/\D/g, "").slice(0, 6))} onKeyDown={e => e.key === "Enter" && verifyOTP()}
              ...
              <button onClick={() => { setStep("choose"); setOtp(""); setError(""); }} style={{ width: "100%", background: "transparent", border: `1px solid ${T.border}`, borderRadius: 8, padding: "10px", cursor: "pointer", fontSize: 13, color: T.textMuted, fontFamily: "sans-serif" }}>← পেছনে যান</button>
            </div>
          )}
        </div>
      </div>
